...
async def run_gallery_dl_fallback(client: Client, message: Message, processing_msg: Message, url: str):
    await processing_msg.edit_text("📸 Instagram detected. Using gallery-dl...")
    ...
    # Add credentials fallback if available
    ...
    try:
        ...
        await processing_msg.edit_text(f"📤 Uploading {len(downloaded_files)} items...")
        ...
        if images:
            if len(images) == 1:
                ...
                for i in range(0, len(images), 10):
                    chunk = images[i:i+10]
                    await asyncio.gather(*[client.send_photo(message.chat.id, img) for img in chunk])
        ...
        await processing_msg.edit_text("❌ Failed to process Instagram content.")
    finally:
    ...
    if downloaded:
        ...
        if images:
            if len(images) == 1:
                ...
                for i in range(0, len(images), 10):
                    await asyncio.gather(*[client.send_photo(message.chat.id, img) for img in images[i:i+10]])
                    await asyncio.sleep(1)
        for vid in videos:
            await client.send_video(message.chat.id, vid, reply_to_message_id=message.id)
        ...
        if is_instagram_url(original_url):
            await run_gallery_dl_fallback(client, message, processing_msg, original_url)
        else:
...
async def download_video_callback(client: Client, callback: CallbackQuery):
    _, format_id, proc_msg_id, user_id_str = callback.data.split(":", 3)
    if callback.from_user.id != int(user_id_str):
        return await callback.answer("Not for you!", show_alert=True)
    ...
    state = user_data.get(storage_key)
    if not state:
        return await callback.edit_message_text("❌ Session expired.")
